...
in {
  ...
  config = mkIf cfg.enable {
    systemd.services."${service_name}-station" = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = "${pkg}/bin/divera-reports --config ${cfg.config_path} reports station --upload Verbesserungsvorschläge_Feuerwehrhaus.xlsx";
    ...
    systemd.services."${service_name}-roster" = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = "${pkg}/bin/divera-reports --config ${cfg.config_path} reports roster --upload Vorschläge_Dienstplan.xlsx";
        ProtectHome = "read-only";
      };
    };
    ...
    systemd.services."${service_name}-absences" = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = "${pkg}/bin/divera-reports --config ${cfg.config_path} reports absences --upload Abwesenheiten.xlsx";
        ProtectHome = "read-only";
      };
    };
